...
2. 지문 작성
- 본문: 학년별 읽기 수준에 맞춰 핵심 개념을 설명하며, 각 단락은 자연스럽게 연결되어야 합니다.
- 어휘 풀이: 과제 핵심어휘의 개념어에 대해서만 풀이를 제공하며, 단락 별로 추출합니다(단락별 최소2개~최대5개, 총 8~25개).

###입력 조건
1. 지문 작성: 입력받은 구분·과목·학년·영역·지문 길이에 맞춰 생성합니다.
- 제목: 질문형 또는 호기심을 유발하는 형태로 작성 (예: "왜 바다는 파란색일까?", "우리 몸의 신비한 면역 시스템")
- 본문: 각 단락이 논리적으로 연결되며, 단락별로 2~5개의 학습 어휘를 포함
- 핵심 개념어는 20개 이상 최대 25개까지 추출하여 어휘 풀이를 제공
- 난이도는 학년별로 조정하되, 개념은 정확하게 전달

2. 질문형 제목 예시
- 초등 중학년: "왜 ~할까?", "어떻게 ~할까?", "무엇이 ~일까?"
- 초등 고학년: "~는 어떤 원리일까?", "~의 비밀은 무엇일까?"
- 중학생: "~는 어떻게 작동하는가?", "~의 메커니즘은 무엇인가?"

3. 실생활 연계
- 각 개념을 일상생활의 구체적 예시와 연결
- 학생들이 직접 경험할 수 있는 상황 제시`,
  ...
  // ============== 2. 어휘 문제 생성 ==============
  {
    ...
    name: '전체 시스템 프롬프트',
    key: 'system_base',
    promptText: `어휘 개념 이해 문제를 생성합니다. 제공된 어휘 목록에서 각 용어의 정의를 묻는 5지선다 객관식 문제를 만들어주세요.

문제 생성 규칙:
1. 각 용어당 1개의 문제 생성
2. 문제는 "다음 중 [용어]의 의미로 가장 적절한 것은?" 형식
3. 정답은 제공된 정의를 바탕으로 작성
4. 오답은 그럴듯하지만 틀린 내용으로 구성
5. 해설은 정답인 이유와 핵심 개념을 간단히 설명`,
    ...
    promptText: `다음 JSON 형식으로 어휘 문제를 생성하세요:
{
  ...
  // 전체 시스템 프롬프트
  {
    ...
    name: '전체 시스템 프롬프트',
    key: 'system_base',
    promptText: `문단별 독해 문제를 생성합니다. 각 문단의 내용을 깊이 이해하고 있는지 확인하는 문제를 만들어주세요.

문제 생성 규칙:
1. 각 문단의 핵심 내용을 파악하는 문제
2. 문단별로 지정된 유형의 문제 1개씩 생성
3. 문단의 내용을 벗어나지 않는 범위에서 출제
4. 학년 수준에 맞는 난이도로 조정`,
...
출력 형식:
{
  ...
  // 전체 시스템 프롬프트
  {
    ...
    name: '전체 시스템 프롬프트',
    key: 'system_base',
    promptText: `전체 지문을 종합적으로 이해하고 있는지 확인하는 문제를 생성합니다.

문제 생성 규칙:
1. 지문 전체의 내용을 아우르는 문제
2. 각 유형별로 정해진 개수만큼 생성
3. 단순 암기가 아닌 이해와 적용을 평가
4. 학년 수준에 맞는 난이도 조정`,
