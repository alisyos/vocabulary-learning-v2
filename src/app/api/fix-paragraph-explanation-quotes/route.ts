...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 paragraph_questions.explanation 큰따옴표 → 작은따옴표 변환 시작...');
    ...
    console.log(`🎯 paragraph_questions.explanation 큰따옴표 → 작은따옴표 변환 완료: 총 성공 ${successCount}개, 실패 ${errorCount}개`);

    return NextResponse.json({
      success: true,
      dryRun: false,
      message: `paragraph_questions.explanation 변환 완료: ${successCount}개 성공, ${errorCount}개 실패`,
      totalRecords: allRecords.length,
      totalWithDoubleQuotes: recordsWithDoubleQuotes.length,
      needsUpdate: updatesNeeded.length,
      successCount,
    ...
    console.error('paragraph_questions.explanation 변환 중 오류:', error);
    return NextResponse.json({
      success: false,
