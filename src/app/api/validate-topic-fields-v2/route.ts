...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. main_topic + sub_topic + grade 조합으로 그룹화
    const groupedByTopicAndGrade = new Map<string, any[]>();
    ...
    for (const record of allSets) {
      ...
      // ✅ grade 추가: main_topic + sub_topic + grade 조합
      const key = `${main_topic}|||${sub_topic}|||${grade}`;
      if (!groupedByTopicAndGrade.has(key)) {
        groupedByTopicAndGrade.set(key, []);
      }
      groupedByTopicAndGrade.get(key)!.push(record);
    }

    console.log(`📊 총 ${groupedByTopicAndGrade.size}개의 main_topic + sub_topic + grade 조합 발견`);

    // 3. 각 그룹에서 필드 값 불일치 검사
    const fieldsToCheck = ['division', 'passage_length', 'text_type', 'session_number', 'grade_number'];
    const inconsistencies: any[] = [];

    for (const [key, records] of groupedByTopicAndGrade) {
      const [main_topic, sub_topic, grade] = key.split('|||');
    ...
    console.log(`⚠️ ${inconsistencies.length}개의 main_topic + sub_topic + grade 조합에서 불일치 발견`);
    ...
    if (dryRun) {
      return NextResponse.json({
        ...
        totalGroups: groupedByTopicAndGrade.size,
        inconsistentGroups: inconsistencies.length,
        totalRecordsToUpdate,
        inconsistencies: inconsistencies.slice(0, 50) // 최대 50개 샘플
      });
    }
